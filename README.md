...
**Timeline:** Oct (MASM kickoff) → Dec (M1) → Jan (DX12, M2) → Feb (rendering pipeline) → Apr (optimization) → May–Jun (M3).
...
`main.cpp` is deliberately tiny — everything else lives behind module/class boundaries (renderer, preset system, asset pipeline).

| Pros | Trade-offs |
|---|---|
| Full control from the assembly window up | Windows / x64 only |
| Bindless + GPU-driven → minimal CPU draw cost | Deferred can't use hardware MSAA (solved with TAA) |
| Modular render graph, passes are reorderable | Higher G-buffer memory/bandwidth |
| C++20 modules give clean compilation boundaries | Module + MASM toolchain needs care |
...
```
BitForge/
|- BitForge/            Main.cpp, Renderer/, Rendering/ (passes), Camera/,
|                       Assets/, Config/Presets/, Scene/, Core/, Input/, UI/
|- WorkDirectory/       source assets (Shaders, Models, Sponza, Textures) + *.asm
|- lib/Window/          bf.Window C++20 module (IXX/ + CPP/)
+- x64/<Config>/        build output (exe + copied Assets/)
```

`WorkDirectory/Assets` is the single source of truth; shaders are compiled from `.hlsl` at runtime via `D3DCompileFromFile`.

---

## Building & controls

**Requires:** Windows 10/11 with a D3D12 GPU · Visual Studio 2022/2025 (v143+) with the Desktop C++ workload + MASM · Windows 10 SDK.

Open `BitForge.slnx`, pick **x64** + Debug/Release, build (the post-build step copies assets next to the exe), run.

```powershell
msbuild BitForge\BitForge.vcxproj /t:Rebuild /p:Configuration=Release /p:Platform=x64 /p:SolutionDir=<repo>\
```

> Release uses Whole Program Optimization + modules — prefer **Rebuild**. After editing a shader, build so the runtime copy refreshes.
...
Cleaner DX12 architecture · deeper raycast integration · more SIMD · scaling to 1,000+ objects · collisions, skeletal animation and physics · Qt viewport with hierarchy, profiler and gizmo tooling.
